 ...
 */
export const validateDateTimeFormat = (dateStr: string, timeStr: string): boolean => {
 ...
 */
export const validateBookingData = (data: any): { valid: boolean, message: string } => {
  ...
  if (!validateDateTimeFormat(startDate, startTime)) {
    return {
      valid: false,
  ...
  if (!validateDateTimeFormat(endDate, endTime)) {
    return {
      valid: false,
